...
export const CanvasView: React.FC<CanvasViewProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] right-4 sm:bottom-6 sm:right-6 flex items-center gap-1.5 md:gap-2 bg-card/65 backdrop-blur-xl border border-border pl-3 pr-4 md:pl-5 md:pr-6 py-2 rounded-full shadow-2xl z-50 select-none max-w-[calc(100%-2rem)] overflow-x-auto flex-nowrap no-scrollbar">
        <button
          type="button"
          onClick={() => addNewCard('text')}
          className="h-8 px-2.5 md:px-3.5 rounded-full flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground bg-muted border border-border hover:bg-border/60 focus:outline-none transition-premium cursor-pointer shrink-0"
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="w-auto md:w-[125px] h-8 px-2.5 md:px-3.5 rounded-full text-xs font-semibold text-muted-foreground hover:text-foreground bg-muted border border-border focus:outline-none transition-premium cursor-pointer flex items-center justify-center md:justify-between text-left shrink-0"
            title="Add Linked Note"
          >
            <div className="flex items-center gap-1.5 truncate">
              <FileText size={13} className="shrink-0" />
              <span className="hidden md:inline truncate">Add Note</span>
            </div>
            <ChevronDown className={`hidden md:block w-3.5 h-3.5 text-muted-foreground shrink-0 transition-transform duration-200 ${isAddNoteOpen ? 'transform rotate-180' : ''}`} />
          </button>
        </div>
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="w-auto md:w-[130px] h-8 px-2.5 md:px-3.5 rounded-full text-xs font-semibold text-muted-foreground hover:text-foreground bg-muted border border-border focus:outline-none transition-premium cursor-pointer flex items-center justify-center md:justify-between text-left shrink-0"
            title="Add Media / Attachment Card"
          >
            <div className="flex items-center gap-1.5 truncate">
              <Image size={13} className="shrink-0" />
              <span className="hidden md:inline truncate">Add Image/PDF</span>
            </div>
            <ChevronDown className={`hidden md:block w-3.5 h-3.5 text-muted-foreground shrink-0 transition-transform duration-200 ${isAddImageOpen ? 'transform rotate-180' : ''}`} />
          </button>
        </div>
        ...
        <span className="text-[0.75rem] font-bold text-muted-foreground px-1 select-none shrink-0 hidden md:inline">
          {Math.round(zoom * 100)}%
        </span>
        ...
        <button
          ...
          title="Save Canvas"
          className={cn(
            "flex items-center justify-center gap-1.5 h-8 w-8 md:w-auto px-0 md:px-4 rounded-full font-semibold text-xs transition-all duration-200 transform shrink-0 cursor-pointer",
            hasUnsavedChanges
